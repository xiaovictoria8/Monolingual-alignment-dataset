...
def print_average_stats(worker_dict, ave_stats_writer):
    ...
    # stats[0] = # of workers, stats[1] = # of HITs, stats[2] = average precision (by HITs), stats[3] = average recall (by HITs)
    # stats[4] = average f1 (by HITs), stats[5] = average precision (by worker), stats[6] = average recall (by worker), stats[7] = average f1 (by worker)
    ...
    for worker in worker_dict:
        try:
            ...
            if qual_type not in qual_type_dict:
                ...
                stats = [1, num_hits, worker_list[2] * num_hits, worker_list[3] * num_hits, worker_list[4] * num_hits, worker_list[2], worker_list[3], worker_list[4]]
            ...
            # the case where we've already seen workers of this qual type 
                ...
                for i in range(5, 8):
                    stats[i] = float(stats[i]) + ((worker_list[i - 3] - stats[i]) / float(stats[0]))
                print(stats)   
        except:
            pass
...
def main():
    ...
    ave_stats_writer.writerow(["qual_type", "num_workers", "num_HITs_completed", "precision (by HITs)", "recall (by HITs)", "f1 (by HITs)", "precision (by worker)", "recall (by worker)", "f1 (by worker)"])
